...
A native `imageView` item projects as a terminal `viewed_image` item carrying
only the path's file name, and that item is never mutated. Live, history, and
page projections and their coordinate indexes reserve two source-order
positions for every `imageView`: the `viewed_image` item and an optional final
image child at its position + 1, whose item ID is the hashed coordinate with an
`image` subkey. Later items keep their order whether or not capture succeeds,
and an unused position never becomes a browser item. Publication identity, the
shared capture service, authority, and snapshot meaning are in [Provider output
artifacts](../output-artifacts.md#codex-viewed-image-capture). Capture uses the
thread's persisted binding; provider-assigned first send, retry, and recovery
reuse that durable binding, including its creation time, rather than a
provisional copy.

A completed `viewed_image` item without a retained child becomes a candidate
only when the planning pass can reserve the child's per-turn item slot and
serialized descriptor bytes. Transcript items take precedence; without room for
every eventual child, that pass schedules no capture and leaves the views
childless rather than failing, shrinking, or replacing the projection.
Incremental live appends that would need reserved capacity release those
reservations instead of forcing a resnapshot; a view whose child no longer fits
stays childless.
...
reprojects childless views from current cached native state, rechecks capacity,
and inserts each new child by source order, including into completed turns. It
emits every added child's `item_completed` before the affected `turn_updated`;
a duplicate emits nothing, and a child that no longer fits stays retained for a
later read. Live reprojection never absorbs a retained child that subscribers
have not received: it keeps the view childless and installs the child with its
own events.
