    ...
    worktrees: z
      .array(
        z.strictObject({
          ...
          provenanceKind: z.enum(["same", "contained", "unmerged", "unknown"]),
...
export const workspaceFilesRemoveLinkedWorktreeOperation = definition({
  operation: "worktrees.remove",
  requestSchema: z.strictObject({
    operationId: z.string().uuid(),
    rootHandle: rootHandleSchema,
    canonicalCheckoutPath: absolutePathSchema,
  ...
  responseSchema: z.strictObject({ removed: z.literal(true) }),
  maximumDeadlineMilliseconds: 60_000,
});

export const workspaceFilesMutationListOperation = definition({
...
export const workspaceFilesMutationInspectOperation = definition({
  operation: "mutation.inspect",
  requestSchema: z.strictObject({ operationId: z.string().uuid() }),
  responseSchema: z.discriminatedUnion("state", [
    ...
    z.strictObject({
      ...
      result: z.union([
        workspaceFilesWriteOperation.responseSchema,
        workspaceFilesRemoveLinkedWorktreeOperation.responseSchema,
      ]),
    }),
  ]),
});
export const workspaceFilesMutationAcknowledgeOperation = definition({
...
export interface WorkspaceFilesV7Handlers {
  ...
  readonly mutationAcknowledge: HandlerFor<
    typeof workspaceFilesMutationAcknowledgeOperation
  >;
  ...
  >;
  readonly removeLinkedWorktree: HandlerFor<
    typeof workspaceFilesRemoveLinkedWorktreeOperation
  >;
...
export function registerWorkspaceFilesV7Operations(
  ...
  registry.register(workspaceFilesMutationListOperation, handlers.mutationList);
  registry.register(
    workspaceFilesMutationInspectOperation,
