...
/**
 * Makes the durable submission payload match the normalized composer contract:
 * text may be empty only when the same payload carries a selected skill.
 */
export const skillOnlyInputMigration: DatabaseMigration = {
  version: 37,
  name: "skill-only-input",
  requiresForeignKeysDisabled: true,
  verifyDatabaseIntegrity: true,
  sql: `
CREATE TABLE conversation_creation_attempts_v37 (
  ...
  fork_context_boundary_state TEXT CHECK (
    fork_context_boundary_state IS NULL
    OR fork_context_boundary_state IN ('pending', 'applying', 'applied', 'unknown')
  ),
  ...
  CHECK (
    (creation_kind = 'first_input'
      AND source_kind IN ('composer', 'automation'))
    OR (creation_kind = 'fork'
      AND source_kind IN ('user_fork', 'automation'))
  ),
  CHECK (
    creation_kind <> 'fork'
  ...
  CHECK (
    (source_kind = 'composer'
      AND (consumed_draft_revision IS NOT NULL
  ...
  CHECK (
    ...
    OR (retry_mutation_id IS NOT NULL
...
INSERT INTO conversation_creation_attempts_v37 (
  tenant_id, owner_principal_id, application_thread_id, attempt_id,
  ...
  reconciliation_token, retry_anchor, retry_authorized_at,
  retry_mutation_id, retry_started_at, retry_reconciliation_token,
  backend_correlation, completion_identity, diagnostic, prepared_at,
  external_call_started_at, accepted_at, reconciled_at,
  fork_child_identity, fork_creation_recovery, fork_uncertainty_kind,
  fork_context_boundary_version, fork_context_boundary_state, initial_skill_id
)
SELECT
  tenant_id, owner_principal_id, application_thread_id, attempt_id,
  mutation_id, backend_instance_id, connection_profile_id,
...
ALTER TABLE conversation_creation_attempts_v37
...
CREATE TABLE queued_inputs_v37 (
  tenant_id TEXT NOT NULL,
  owner_principal_id TEXT NOT NULL,
  ...
  text TEXT NOT NULL CHECK (
    length(CAST(text AS BLOB)) BETWEEN 0 AND 65536
  ),
  ...
  invalid_state_requeues INTEGER NOT NULL DEFAULT 0 CHECK (
    invalid_state_requeues BETWEEN 0 AND 1
  ),
  ...
  trigger_kind TEXT NOT NULL DEFAULT 'user' CHECK (
    trigger_kind IN ('user', 'automation')
  ),
  ...
  ) REFERENCES queued_inputs_v37(
    tenant_id, owner_principal_id, application_thread_id, id
  ) ON DELETE RESTRICT,
  CHECK (length(text) > 0 OR selected_skill_id IS NOT NULL),
...
INSERT INTO queued_inputs_v37 (
  ...
  backend_correlation, retry_count, invalid_state_requeues,
  next_attempt_at, diagnostic, failure_acknowledged_at, trigger_kind,
  source_automation_id, source_automation_run_id, selected_skill_id
)
SELECT
  tenant_id, owner_principal_id, id, application_thread_id, sequence,
  mutation_id, text, state, retry_of_id, created_at, dispatch_started_at,
...
ALTER TABLE queued_inputs_v37 RENAME TO queued_inputs;
...
WHEN NOT (
  ...
  OR (NEW.trigger_kind = 'automation'
    ...
    AND EXISTS (
      ...
      WHERE run.tenant_id = NEW.tenant_id
        ...
        AND (
          ...
          OR (NEW.retry_of_id IS NOT NULL AND EXISTS (
            SELECT 1 FROM queued_inputs AS parent
            WHERE parent.tenant_id = NEW.tenant_id
              AND parent.owner_principal_id = NEW.owner_principal_id
              AND parent.application_thread_id = NEW.application_thread_id
              ...
              AND parent.source_automation_run_id = NEW.source_automation_run_id
          ))
        )
    ))
)
