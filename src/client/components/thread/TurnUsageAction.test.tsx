...
it("does not subscribe to ended-turn availability while experimental accounting is disabled",async()=>{
  const getUsage=vi.fn(), getUsageAvailability=vi.fn();
  const cache=new UsageQueryCache("thread",{getUsage,getUsageAvailability});caches.push(cache);
  const activate=vi.spyOn(cache,"activateAvailability");
  render(<TurnUsageAction cache={cache} turn={{id:"turn",revision:1,status:"completed",orderedItemIds:[]}} onOpenChange={vi.fn()}/>);
...
it("hides running and empty turns, then shows the action after durable usage arrives",async()=>{
  ...
  const cache=new UsageQueryCache("thread",{getUsage,getUsageAvailability});cache.setEnabled(true);caches.push(cache);
