...
describe("Claude native usage accounting", () => {
  ...
  it("reuses actual query epochs and keeps history independent of resumed query checkpoints", () => {
    ...
    const sink: UsageSink = {findSubagent: () => null, listSubagentRoots: () => ({bindings:[],nextCursor:null}), listSubagents: () => [], open: (source) => {epochs.push(source.epoch); return {registerTurns: () => {}, capture: (entries) => { captured.push(...entries); return true; }, gap: () => {}, reconcile: () => true, seal: (reason) => sealed.push(reason)};}};
  ...
  it("reports each delivery transaction outcome for existing replay acknowledgement", () => {
    let durable=false;
    const sink:UsageSink={findSubagent: () => null, listSubagentRoots: () => ({bindings:[],nextCursor:null}), listSubagents: () => [], open:()=>({registerTurns:()=>{},capture:()=>durable,gap:()=>{},reconcile:()=>true,seal:()=>{}})};
