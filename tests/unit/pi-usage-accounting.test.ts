...
describe("Pi native usage accounting", () => {
  ...
  it("captures inactive branches and multiple calls under their native turn", () => {
    ...
    const sink: UsageSink = {findSubagent: () => null, listSubagentRoots: () => ({bindings:[],nextCursor:null}), listSubagents: () => [], open: () => ({registerTurns: () => {}, capture: (entries) => { captured.push(...entries); return true; }, gap: () => {}, reconcile: () => true, seal: () => {}})};
  ...
  it("marks authenticated copied turns as inherited while capturing only new child work", () => {
    ...
    const sink:UsageSink={findSubagent: () => null, listSubagentRoots: () => ({bindings:[],nextCursor:null}), listSubagents: () => [], open:()=>({registerTurns:(_turns,origin)=>{if(origin)inherited.push(origin);},capture:(observations)=>{captured.push(...observations);return true;},gap:()=>{},reconcile:()=>true,seal:()=>{}})};
  ...
  it("does not declare native history reconciled after invalid evidence or a failed commit", () => {
    ...
    new PiUsageAccounting({...input,manager:validManager,sink:{findSubagent: () => null, listSubagentRoots: () => ({bindings:[],nextCursor:null}), listSubagents: () => [], open:()=>({...NO_USAGE_CAPTURE,reconcile,capture:()=>false})}});
