...
describe.sequential("real Claude subscription driver", () => {
  ...
  database.exec(claudeSteerOperationsMigration.sql);
    ...
    const driver = new ClaudeConversationBackendDriver({
      usage: {enabled: true, findSubagent: () => null, listSubagentRoots: () => ({bindings:[],nextCursor:null}), listSubagents: () => [], open: () => ({registerTurns: () => {}, capture: (entries) => { usageObservations.push(...entries); return true; }, gap: () => {}, reconcile: () => true, seal: () => {}})},
